...
**Security Warning:** Reading from `location.search` and injecting values directly into hidden inputs can enable DOM-based XSS attacks unless UTM values are properly validated and encoded before DOM insertion. Attackers can craft malicious URLs with JavaScript payloads in UTM parameters that execute when injected into the page. Prefer using your marketing platform's native UTM capture mechanisms, or ensure all UTM values are sanitized and HTML-encoded before insertion into form fields or the DOM.
