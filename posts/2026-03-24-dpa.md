...
- “You” or “customer” refers to the company or organization that signs up to use Swetrix to analyze website visitors, track errors, use optional session replays, or use CAPTCHA.
- “Service” refers to the Swetrix analytics, error tracking, optional session replay, and CAPTCHA platform.
...
We take multiple measures to protect and secure your data through backups, redundancies, and encryption. When you use our service to measure your website traffic, track errors, record optional session replays, or serve CAPTCHAs, Swetrix will collect information about your visitors ("End Users").
...
By using Swetrix for standard site measurement, analytics is carried out in an anonymous and privacy-friendly way. We minimize data collection in general. We measure only the most essential data points and nothing else.

For standard analytics, we do not use cookies, browser cache nor local storage for tracking visitors. We do not store, retrieve nor extract anything from visitor devices without consent.
...
Session replays are an optional Swetrix Cloud feature. A regular pageview, custom event, or error event does not create a replay. Recording starts only when the customer calls `startSessionReplay()` on its website or application. Depending on the privacy mode and masking options selected by the customer, replay data may include page state, user interactions, DOM changes, and non-password input values. The customer is responsible for deciding when replay recording is lawful, configuring masking and page exclusions, and obtaining any required consent or other legal basis before recording End Users.
...
- Swetrix will assist the customer with data protection obligations and forward any data subject requests to the customer. Where the Service does not hold identifiable data allowing us to process such requests directly, the customer remains responsible for locating and actioning the relevant request within its account.
...
- The Customer agrees not to pass any personal data, Personally Identifiable Information (PII), or Protected Health Information (PHI) to Swetrix through URLs, custom events, metadata, error messages, or other configured analytics fields.
- For optional session replays, the Customer is responsible for excluding sensitive pages, choosing appropriate privacy and masking settings, obtaining any required consent or other legal basis, and not recording PHI or other highly sensitive data unless Swetrix has agreed to that processing in writing.
